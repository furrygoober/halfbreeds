...
(defcard "Remote Connection"
  {:req (req (some #{:hq :rd :archives} (:successful-run runner-reg))) ;; ✅ FIX
   ...
   :abilities [{:action true
                ...
                :async true
                :effect
                (req (let [initial-server (:card-target (get-card state card))]
                       (register-events state side card
                         [{:event :pre-successful-run
                           :duration :end-of-run
                           :unregister-once-resolved true
                           ...
                           :effect
                           (req
                             (swap! state assoc-in [:run :server] [(unknown->kw target)])
                             ...
                             (let [reveal-target (first (shuffle (:hand corp)))]
                               (system-msg state :runner
                                 (str "uses " (:title card)
                                      " to force the Corp to reveal "
                                      (:title reveal-target) " from HQ"))
